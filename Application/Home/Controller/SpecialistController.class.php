...
class SpecialistController extends Controller {
    ...
    protected function _initialize(){
		...
		/* 读取栏目信息 */
		$Category_info = M('category') -> where('name="specialist"') -> select();
		//dump($Category_info);
    ...
    public function index(){
		...
		//联合查询
		$list = $db -> field( "$a.id, $a.title, $b.intro, $b.photo" ) ->
        join( "$b on $a.id = $b.id" ) ->
        where( "status = 1" ) ->
    ...
    public function article($article_id){
		...
		$b = "tth_document_specialist"; //表2
		//联合查询
		$Article_info = $db -> field( "$a.id, $a.title, $a.category_id, $a.description, $b.keywords, $b.introduce, $b.photo, $b.eng_name" ) ->
        join( "$b on $a.id = $b.id" ) ->
		...
		/* 读取相关专家 */
		$unite = $db -> field( "$a.id, $a.title, $b.photo" ) ->
		join( "$b on $a.id = $b.id" ) ->
		where( "status = 1" ) ->
		order('level DESC, id DESC')->
		limit(8) ->
